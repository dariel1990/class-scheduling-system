...
class ClassRoomAvailabilityRule implements ValidationRule
{
    protected $timeslot;
    ...
    public function validate(string $attribute, mixed $value, Closure $fail): void
    {
        ...
        $available = ClassSchedule::where(function ($query) use ($value) {
            $query->whereHas('subject_assignments', function ($subQuery) use ($value) {
                $subQuery->where('class_id', '=', $value);
            });
        ...
        if (!$available) {
            $fail('The Class is not available at the specified time');
        }
    }
}
